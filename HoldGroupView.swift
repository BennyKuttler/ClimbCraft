        ...
        VStack(spacing: 20) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 20) {
                ForEach(folder.holdGroups, id: \.self) { holdGroup in
                    NavigationLink(destination: HoldView(folder: folder, holdGroup: holdGroup)) {
                        VStack {
                            Image(holdGroup)
                                .resizable()
                                .scaledToFit()
...
protocol HoldViewDelegate: AnyObject {
    func didSelectHold(_ hold: Folder.Hold)
}
        ...
        ScrollView {
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 20) {
                ForEach(holdsForGroup, id: \.id) { hold in
                    NavigationLink(destination: WallView(selectedWallImage: selectedWallImage, selectedHoldImage: UIImage(named: hold.name), selectedHold: hold).navigationBarBackButtonHidden(true)) {
                        VStack {
                            Image(hold.name)
                                .resizable()
